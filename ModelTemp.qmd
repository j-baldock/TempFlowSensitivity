...
Post-hoc site specific intercepts
```{r}
ggfit %>% 
...
cat("model {
    ...
    for (i in 1:nFirstObsRows){
      temp[firstObsRows[i]] ~ dnorm(stream.mu[firstObsRows[i]], pow(sigma, -2)) 
      stream.mu[firstObsRows[i]] <- trend[firstObsRows[i]]
      trend[firstObsRows[i]] <- inprod(B.0[], X.0[firstObsRows[i], ]) + inprod(B.river[river[firstObsRows[i]], ], X.river[firstObsRows[i], ]) + inprod(B.year[year[firstObsRows[i]], ], X.year[firstObsRows[i], ])
    ...
    for (i in 1:nEvalRows){ 
      temp[evalRows[i]] ~ dnorm(stream.mu[evalRows[i]], pow(sigma, -2))
      stream.mu[evalRows[i]] <- trend[evalRows[i]] + ar1[river[evalRows[i]]] * (temp[evalRows[i]-1] - trend[ evalRows[i]-1 ])
      trend[evalRows[i]]  <- inprod(B.0[], X.0[evalRows[i], ]) + inprod(B.river[river[evalRows[i]], ], X.river[evalRows[i], ]) + inprod(B.year[year[evalRows[i]], ], X.year[evalRows[i], ])
      }
    ...
    for (i in 1:nRiver){
      B.river[i,1] ~ dnorm(mu.B.river[1], pow(sigma.muB[1],-2) )  
      B.river[i,2] ~ dnorm(mu.B.river[2], pow(sigma.muB[2],-2) )  
    }
    mu.B.river[1] ~ dnorm(0, pow(100,-2))
    mu.B.river[2] ~ dnorm(0, pow(100,-2))
    sigma.muB[1] ~ dunif(0.00001, 100) 
    sigma.muB[2] ~ dunif(0.00001, 100) 
    ...
    }", file = "DailyTempModelJAGS_Letcher_hierarchical.txt")
```
...
```{r eval=FALSE}
fit0_h <- jags(data = jags.data, inits = NULL, parameters.to.save = jags.params, model.file = "DailyTempModelJAGS_Letcher_hierarchical.txt",
            n.chains = 3, n.thin = 5, n.burnin = 1500, n.iter = 3500, DIC = TRUE)
```
...
```{r}
ggs_caterpillar(ggfit, family = "mu.B.river")
```
...
ggfit %>%
  ...
  filter(Parameter %in% c("B.river[1,1]", "B.river[2,1]", "B.river[3,1]", "B.river[4,1]")) %>%
  mutate(Parameter = factor(Parameter, levels = c("B.river[1,1]", "B.river[2,1]", "B.river[3,1]", "B.river[4,1]"))) %>%
  ggs_caterpillar(sort = FALSE) +
...
ggfit %>%
  filter(Iteration > 125) %>% 
  filter(Parameter %in% c("B.river[1,2]", "B.river[2,2]", "B.river[3,2]", "B.river[4,2]")) %>%
  mutate(Parameter = factor(Parameter, levels = c("B.river[1,2]", "B.river[2,2]", "B.river[3,2]", "B.river[4,2]"))) %>%
  ggs_caterpillar(sort = FALSE) +
...
Similar hierarchical structure as above, but allow effect of flow and flow-temp interaction to vary by site. 
...
cat("model {
    ...
    # random effect coefficients
    for (i in 1:nRiver){
      B.river[i,1] ~ dnorm(mu.B.river[1], pow(100,-2) )  
      B.river[i,2] ~ dnorm(mu.B.river[2], pow(100,-2) )  
      B.river[i,3] ~ dnorm(mu.B.river[3], pow(100,-2) )  
      B.river[i,4] ~ dnorm(mu.B.river[4], pow(100,-2) )  
    }
    mu.B.river[1] ~ dnorm(0, pow(100,-2))
    mu.B.river[2] ~ dnorm(0, pow(100,-2))
    mu.B.river[3] ~ dnorm(0, pow(100,-2))
    mu.B.river[4] ~ dnorm(0, pow(100,-2))
...
```{r eval=FALSE}
fit0_h2 <- jags(data = jags.data, inits = NULL, parameters.to.save = jags.params, model.file = "DailyTempModelJAGS_Letcher_hierarchical.txt",
            n.chains = 3, n.thin = 5, n.burnin = 1500, n.iter = 3500, DIC = TRUE)
```
